...
if (!localStorage.getItem(activeAccountStorageKey)) {
...
let weeklyRewards = {};
let weeklyQuestIds = [];
let healthyLifeQuestIds = [];
let healthyLifeQuestPage = 0;
let healthyLifeQuestDate = "";
let lastDate = "";
let weeklyWeekKey = "";
let dailyQuestDate = "";
let questStreak = 0;
let accountList = [];
let activeAccountId = localStorage.getItem(activeAccountStorageKey) || "";
...
function createAccount(name = "Hunter") {
    const trimmedName = (name || "Hunter").trim() || "Hunter";
    let finalName = trimmedName;
    let suffix = 2;

    while (accountList.some((account) => account.name.toLowerCase() === finalName.toLowerCase())) {
    ...
    localStorage.setItem(accountListStorageKey, JSON.stringify(accountList));
    localStorage.setItem(getAccountStorageKey(account.id), JSON.stringify(createBlankAccountState(finalName)));
    return account;
}
...
function migrateLegacyStorage() {
    ...
    const migratedState = createBlankAccountState(localStorage.getItem("playerName") || "Hunter");
    migratedState.xp = Number(localStorage.getItem("xp")) || 0;
    migratedState.level = Number(localStorage.getItem("level")) || 1;
    migratedState.stats = {
        ...migratedState.stats,
        ...(JSON.parse(localStorage.getItem("stats")) || {})
    };
    migratedState.substats = {
        ...migratedState.substats,
        ...(JSON.parse(localStorage.getItem("substats")) || {})
    };
    migratedState.quests = JSON.parse(localStorage.getItem("quests")) || {};
    migratedState.weeklyQuestIds = JSON.parse(localStorage.getItem("weeklyQuestIds")) || [];
    migratedState.weeklyProgress = JSON.parse(localStorage.getItem("weeklyProgress")) || {};
    migratedState.weeklyRewards = JSON.parse(localStorage.getItem("weeklyRewards")) || {};
    migratedState.weeklyWeekKey = localStorage.getItem("weeklyWeekKey") || "";
    migratedState.lastDate = localStorage.getItem("lastDate") || "";
    migratedState.dailyQuestIds = JSON.parse(localStorage.getItem("dailyQuestIds")) || [];
    migratedState.dailyQuestDate = localStorage.getItem("dailyQuestDate") || "";
    migratedState.healthyLifeQuestIds = JSON.parse(localStorage.getItem("healthyLifeQuestIds")) || [];
    migratedState.healthyLifeQuestPage = Number(localStorage.getItem("healthyLifeQuestPage")) || 0;
    migratedState.healthyLifeQuestDate = localStorage.getItem("healthyLifeQuestDate") || "";
    migratedState.questStreak = Number(localStorage.getItem("questStreak")) || 0;

    const migratedAccount = createAccount(migratedState.playerName);
    localStorage.setItem(getAccountStorageKey(migratedAccount.id), JSON.stringify(migratedState));

    legacyKeys.forEach((key) => localStorage.removeItem(key));
    return migratedAccount;
}

function ensureAccountList() {
    const rawAccounts = localStorage.getItem(accountListStorageKey);
    if (rawAccounts) {
        try {
            accountList = JSON.parse(rawAccounts);
        } catch (error) {
            accountList = [];
        }
    }
    ...
    if (!Array.isArray(accountList) || !accountList.length) {
        const migratedAccount = migrateLegacyStorage();
        if (migratedAccount) {
            accountList = [migratedAccount];
        } else {
            const fallbackAccount = createAccount("Hunter");
            accountList = [fallbackAccount];
        }
        localStorage.setItem(accountListStorageKey, JSON.stringify(accountList));
    }

    accountList = accountList.filter(Boolean);

    if (!activeAccountId || !accountList.some((account) => account.id === activeAccountId)) {
...
function saveAccountState(accountId = activeAccountId) {
    if (!accountId) return;

    const currentAccount = accountList.find((account) => account.id === accountId);
    if (currentAccount && currentAccount.name !== playerName) {
        currentAccount.name = playerName;
        localStorage.setItem(accountListStorageKey, JSON.stringify(accountList));
    }

    localStorage.setItem(getAccountStorageKey(accountId), JSON.stringify(getCurrentAccountState()));
}

function loadAccountState(accountId) {
    if (!accountId) {
        return createBlankAccountState();
    }
    ...
    try {
        ...
        return {
            ...createBlankAccountState(parsed.playerName || "Hunter"),
            ...parsed,
            stats: {
                ...createBlankAccountState().stats,
                ...(parsed.stats || {})
            },
            substats: {
                ...createBlankAccountState().substats,
                ...(parsed.substats || {})
            }
        };
    } catch (error) {
...
function applyAccountState(state) {
    playerName = state.playerName || "Hunter";
    xp = Number(state.xp) || 0;
    level = Number(state.level) || 1;
    stats = {
        body: Number(state.stats?.body) || 0,
        mind: Number(state.stats?.mind) || 0,
        social: Number(state.stats?.social) || 0,
        wealth: Number(state.stats?.wealth) || 0,
        spirit: Number(state.stats?.spirit) || 0,
        creativity: Number(state.stats?.creativity) || 0
    };
    substats = {
        body: {
            strength: Number(state.substats?.body?.strength) || 0,
            endurance: Number(state.substats?.body?.endurance) || 0,
            agility: Number(state.substats?.body?.agility) || 0
        },
        mind: {
            focus: Number(state.substats?.mind?.focus) || 0,
            learning: Number(state.substats?.mind?.learning) || 0,
            discipline: Number(state.substats?.mind?.discipline) || 0
        },
        social: {
            charisma: Number(state.substats?.social?.charisma) || 0,
            empathy: Number(state.substats?.social?.empathy) || 0,
            teamwork: Number(state.substats?.social?.teamwork) || 0
        },
        wealth: {
            saving: Number(state.substats?.wealth?.saving) || 0,
            career: Number(state.substats?.wealth?.career) || 0,
            investing: Number(state.substats?.wealth?.investing) || 0
        },
        spirit: {
            meditation: Number(state.substats?.spirit?.meditation) || 0,
            gratitude: Number(state.substats?.spirit?.gratitude) || 0,
            purpose: Number(state.substats?.spirit?.purpose) || 0
        },
        creativity: {
            imagination: Number(state.substats?.creativity?.imagination) || 0,
            expression: Number(state.substats?.creativity?.expression) || 0,
            innovation: Number(state.substats?.creativity?.innovation) || 0
        }
    };
    savedQuests = state.quests || {};
    weeklyQuestIds = Array.isArray(state.weeklyQuestIds) ? state.weeklyQuestIds : [];
    weeklyProgress = state.weeklyProgress || {};
    weeklyRewards = state.weeklyRewards || {};
    weeklyWeekKey = state.weeklyWeekKey || "";
    lastDate = state.lastDate || "";
    dailyQuestIds = Array.isArray(state.dailyQuestIds) ? state.dailyQuestIds : [];
    dailyQuestDate = state.dailyQuestDate || "";
    healthyLifeQuestIds = Array.isArray(state.healthyLifeQuestIds) ? state.healthyLifeQuestIds : [];
    healthyLifeQuestPage = Number(state.healthyLifeQuestPage) || 0;
    healthyLifeQuestDate = state.healthyLifeQuestDate || "";
...
function refreshAccountSelector() {
    ...
    const selectedId = activeAccountId || accountList[0]?.id || "";
    select.innerHTML = "";
...
function switchAccount(accountId) {
    ...
    const state = loadAccountState(activeAccountId);
...
function initializeCurrentAccount() {
    ...
    if (weeklyWeekKey !== currentWeekKey || !isValidWeeklyQuestSet(weeklyQuestIds)) {
...
function setupNameInput() {
    ...
    if (createButton && newAccountInput) {
        createButton.addEventListener("click", () => {
            const newName = newAccountInput.value.trim() || "Hunter";
